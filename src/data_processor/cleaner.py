...
- Support for Core Utility metrics (Volume, Borrowed, Net Inflow, Circulating Supply)
- Support for Liquidity metrics (Liquidity = Operational Output / TVL)

5-Dimension Framework Support:
- D1: Capital (TVL)
- D2: Liquidity (Operational Output / TVL - protocol-specific)
- D3: User Activity (DAU, Tx Count)
- D4: Operational Output (Core Utility - protocol-specific)
...
class DataCleaner:
    ...
    def load_protocol_data(self, protocol):
        ...
        # Trading Volume (for DEX protocols)
        path_volume = self.raw_api_dir / f"volume_{protocol}.csv"
        df_volume = self.read_and_standardize(path_volume, 'volume')
        if df_volume is not None:
            protocol_df = protocol_df.join(df_volume)

        # Active Loans (for lending protocols)
        path_loans = self.raw_manual_dir / f"{protocol}_active_loans.csv"
        df_loans = self.read_and_standardize(path_loans, 'active_loans')
        if df_loans is not None:
            protocol_df = protocol_df.join(df_loans)

        # Assets Staked (for LSD protocols - Lido)
        # Note: Net Inflow will be computed in feature_engineer as:
        # Net Inflow_t = Staked Assets_t - Staked Assets_{t-1}
        # Used for: Core Utility (D4) and Liquidity (D2: Staking Flow = Net Inflow / TVL)
        path_staked = self.raw_manual_dir / f"{protocol}_assets_staked.csv"
        df_staked = self.read_and_standardize(path_staked, 'assets_staked')
        if df_staked is not None:
            protocol_df = protocol_df.join(df_staked)

        # Circulating Supply (for MakerDAO)
        if protocol == 'makerdao':
            path_supply = self.raw_manual_dir / "makerdao_circulating_supply.csv"
            df_supply = self.read_and_standardize(path_supply, 'circulating_supply')
            if df_supply is not None:
                protocol_df = protocol_df.join(df_supply)
        ...
        if protocol in ['uniswap_v3', 'curve']:
            ...
            # Liquid Staking: Assets Staked (D4 Core Utility)
            # Note: Net Inflow is derived from diff(assets_staked) in feature_engineer
            # Core Utility uses raw staked assets (stock measure)
    ...
    def create_master_dataset(self):
        ...
        # Reorder columns
        cols = ['date', 'protocol', 'tvl', 'fees', 'revenue', 'volume',
                'dau', 'tx_count', 'core_utility', 'active_loans',
                'assets_staked', 'circulating_supply']
