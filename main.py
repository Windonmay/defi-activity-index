...
vitality across four dimensions: Capital Efficiency, User Activity, 
...
# Weighting scheme documentation (actual weights defined in aggregator_optimized.py)
WEIGHTING_SCHEME = """
Optimized Equal-Dimension Weighting Scheme
------------------------------------------
The composite index is constructed using equal weights across four core dimensions (25% each).
Within each dimension, weights are distributed equally among its indicators.

D1: Capital (25%)
  - TVL: 12.5%
  - Capital Turnover Ratio: 12.5%

D2: User Activity (25%)
  - Daily Active Users (DAU): 12.5%
  - Transaction Count (Tx Count): 12.5%

D3: Operational Throughput (25%)
  - Core Utility Metric (e.g., Volume / Borrowed / Supply): 25%

D4: Financial Sustainability (25%)
  - Total Fees: 12.5%
  - Protocol Revenue: 12.5%

Total Weight = 100%

Rationale:
- D3 isolates Core_Utility to prevent penalizing protocols like Compound
  that have strong lending activity but low explicit fee revenue.
- D4 combines Fees and Revenue to capture overall value capture ability.
- Log transformation applied to compress right-skewed distributions.
"""
...
def stage_2_data_cleaning() -> None:
    ...
    4. Compute derived metrics (Capital Turnover Ratio)
    
    Capital Turnover Formula:
        Turnover = Core_Utility / TVL
        
    Where Core_Utility is context-specific:
        - DEX (Uniswap, Curve): Daily trading volume
        - Lending (Aave, Compound): Outstanding borrows
        - Stablecoin (MakerDAO): DAI circulating supply
        - LSD (Lido): Total ETH staked
    ...
    try:
        ...
        print_substep("2", "2", "Engineering features (Capital Turnover)")
...
def stage_3_normalization() -> None:
    ...
    1. Log transformation: log(1 + x) applied to skewed metrics
       ...
       - NOT applied to: Capital_Turnover (already a ratio)
    ...
    DeFi metrics are heavily right-skewed (e.g., Uniswap volume >> MakerDAO).
    Log transformation compresses these differences, ensuring smaller protocols
    are not entirely dominated by larger ones in the composite index.
    ...
    """
    print_stage_header(3, "NORMALIZATION (Optimized)")
    ...
    try:
        ...
        print("    2. 7-day moving average smoothing")
        print("    3. Min-Max scaling to [0, 100]")
        ...
        for col in score_cols[:3]:  # Show first 3
...
def stage_4_index_construction() -> None:
    ...
    Aggregation Formula:
        Index = Σ (Weight_i × Score_i)
    
    This is a linear additive model assuming compensability
    (high performance in one dimension can offset low performance in another).
    
    Outputs
    -------
    data/final/final_index_optimized.csv : Composite index with dimension scores
    """
    print_stage_header(4, "INDEX CONSTRUCTION (Optimized)")
    ...
    try:
        ...
        # Save final index
        output_file = FINAL_DIR / "final_index_optimized.csv"
        ...
        print(f"    Overall mean: {index_df[score_col].mean():.2f}")
        print(f"    Overall std:  {index_df[score_col].std():.2f}")
        print(f"    Min score:    {index_df[score_col].min():.2f}")
        print(f"    Max score:    {index_df[score_col].max():.2f}")
        ...
        for protocol, mean_score in protocol_means.items():
            print(f"    {protocol:15s}: {mean_score:.2f}")
        
        # Latest rankings
        print("\n  Latest Date Rankings:")
        latest_date = index_df['date'].max()
        latest_ranking = index_df[index_df['date'] == latest_date].sort_values(
            score_col, ascending=False
        )[['protocol', score_col]]
        print(f"    Date: {latest_date}")
        for _, row in latest_ranking.iterrows():
            print(f"      {row['protocol']:15s}: {row[score_col]:.2f}")
...
def print_pipeline_summary() -> None:
    ...
    files_to_check = [
        ...
        ("Final Index", FINAL_DIR / "final_index_optimized.csv"),
    ]
    ...
    # Data Summary
    final_index_path = FINAL_DIR / "final_index_optimized.csv"
    if final_index_path.exists():
        ...
        print(f"\n  Index Score Range:")
        print(f"    Minimum: {df[score_col].min():.2f}")
        print(f"    Maximum: {df[score_col].max():.2f}")
